...
def return_dataset(cfg,dataFolder):
    cfg.set.max_antibody_len=max_antibody_len[cfg.set.dataset]
    cfg.set.max_antigen_len=max_antigen_len[cfg.set.dataset]
...
class DeepAAIDataset(Dataset):
    def __init__(self, pair_data,dataFolder,cfg,unique_ab_id,unique_ag_id,all_unique_ab_id,all_unique_ag_id):
        ...
        self.cfg=cfg
        self.max_ab_length = cfg.set.max_antibody_len
        self.max_ag_length = cfg.set.max_antigen_len
        ...
        self.antibody_one_hot = amino_seq_to_one_hot(self.ab_seq.ab_seq.tolist(),max_len=self.max_ab_length)
        self.antigen_one_hot = amino_seq_to_one_hot(self.ag_seq.ag_seq.tolist(),max_len=self.max_ag_length)

        ab_kmer_mat = torch.load(dataFolder + f'ab_kmer.pt')
        # print(ab_kmer_mat.shape)
        ag_kmer_mat = torch.load(dataFolder + f'ag_kmer.pt')
        # print(ab_kmer_mat.shape)

        self.unique_ab_kmer = ab_kmer_mat[self.unique_ab_id]
        self.unique_ag_kmer = ag_kmer_mat[self.unique_ag_id]
...
def k_fold_dataset(cfg, dataFolder,k):
    ...
    if cfg.set.model_name == 'DeepAAI':
        ...
        train_dataset = BaselineDataset(train_data, dataFolder, cfg)
        test_dataset = BaselineDataset(test_data, dataFolder, cfg)
...
def load_dataset(cfg, dataFolder):
    ...
    train_data=pd.read_csv(os.path.join(dataFolder+f'{task}', f'train_seed_{cfg.set.seed}.csv'))
    val_data=pd.read_csv(os.path.join(dataFolder+f'{task}', f'val_seed_{cfg.set.seed}.csv'))
    # test_data=pd.read_csv(os.path.join(dataFolder+f'{task}', f'test_seed_{cfg.set.seed}.csv'))
    print(f"Train number {train_data.shape[0]}")
    print(f"Valid number {val_data.shape[0]}")
    if task != 'transfer':
        unseen_data=pd.read_csv(os.path.join(dataFolder+f'{task}', f'unseen_seed_{cfg.set.seed}.csv'))
    ...
    if cfg.set.model_name == 'DeepAAI':
        ...
        train_dataset = BaselineDataset(train_data, dataFolder, cfg)
        val_dataset = BaselineDataset(val_data, dataFolder, cfg)
        # test_dataset = PairDataset(test_data, dataFolder, cfg)
        if isinstance(unseen_data, pd.DataFrame):
            unseen_dataset = BaselineDataset(unseen_data, dataFolder, cfg)
        else:
            unseen_dataset = None
