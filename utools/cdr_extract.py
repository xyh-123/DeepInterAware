...
#                                   'L': range(89 - NUM_EXTRA_RESIDUES, 98 + NUM_EXTRA_RESIDUES)}):
    ...
    cdr3_scheme={'H':range(95,102),:wq'L':range(89,97)}
    '''
    try :
        import anarci
    except ImportError :
...
def extract_CDR(res_H,res_L):
    ab_info = Munch()
    ab_cdr = ""
    if res_H != None and res_H != "" and pd.notna(res_H):
        # ab_info['res_H'] = res_H
        height_chain = get_CDR_simple(res_H, seqname=" heavy")
    ...
    if res_L != None and res_L != "" and pd.notna(res_L):
        # print(res_L)
