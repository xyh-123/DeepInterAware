from utils.comm_utils import TopKModel,ReduceLROnPlateau,EvaMetric,return_loss
...
class ClsBaseTrainer(object):
    def __init__(self, trainer_parameter):
        ...
        self.scheduler=ReduceLROnPlateau(self.optim.opt,mode='max',patience=3)
...
class Trainer(ClsBaseTrainer):
    def __init__(self, trainer_parameter):
        ...
        self.save_file_path = self.output_dir + f'batch_{self.batch_size}_epoch_{self.max_epoch}_seed_{self.seed}_{self.cfg.set.unseen_task}_dim_{self.cfg.predict.hidden_dim}_embed_dim_{self.cfg.protein.embedding_dim}_{self.cfg.protein.channel}_{self.cfg.protein.pool_type}'
    ...
    def train(self):
        ...
        for i in range(self.init_epoch+1, self.epochs + 1):
            ...
            if topk!=-1:
                ...
                with open(unseen_prettytable_file, "w") as fp:
                    fp.write(self.unseen_test_table.get_csv_string())
                if self.cfg.set.dataset == 'ppi':
                    file = os.path.join(self.save_file_path, "test_species.csv")
                    with open(file, "w") as fp:
                        fp.write(self.species_table.get_csv_string())
    ...
    def train_epoch(self,):
        ...
        if self.cfg.set.model_name == 'DeepAAI':
            ...
            unique_ab_kmer = torch.from_numpy(unique_ab_kmer).to(self.device)
            unique_ag_kmer = torch.from_numpy(unique_ag_kmer).to(self.device)
        ...
        # fetcher = InputFetcher(self.train_dataloader, self.cfg, self.device)
        ...
        for i, batch in pbar:
            ...
            # ab_ids = {k: v.to(self.device) for k, v in ab_ids.items()}
            # ag_ids = {k: v.to(self.device) for k, v in ag_ids.items()}
            # batch.ab_ids = ab_ids
            # batch.ag_ids = ag_ids
            self.step += 1
            ...
            inputs=Munch(
                batch=batch,
                device=self.device,
                mixup=self.cfg.set.mixup,
                mode='train',
                interaction_task=self.cfg.set.interaction_task
            )
            self.optim.opt.zero_grad()
            output = self.model(inputs)
            ...
            lr = self.optim.opt.state_dict()['param_groups'][0]['lr']

            # w2 = torch.exp(self.model.w[1]) / torch.sum(torch.exp(self.model.w))
            # if self.cfg.set.metric_loss==None:
...
class SMAICFTrainer(ClsBaseTrainer):
    def __init__(self, trainer_parameter):
        super(SMAICFTrainer, self).__init__(trainer_parameter)
        self.start_epoch = trainer_parameter.start_epoch
        self.end_epoch = trainer_parameter.end_epoch
    ...
    def train_epoch(self):
        ...
        for i, batch in pbar:
            ...
            step += 1
            if self.start_epoch <= self.current_epoch <= self.end_epoch:
                stage = 1
            else:
                stage = 2
            inputs = Munch(
...
class KFoldMultiSIPTrainer(object):
    def __init__(self, trainer_parameter):
        ...
        self.current_epoch = trainer_parameter.current_epoch
        self.train_dataloader = trainer_parameter.train_dataloader
        self.test_dataloader = trainer_parameter.val_dataloader
        ...
        self.output_dir = cfg.result.output_dir + '/' + cfg.set.dataset + '/' + self.model_name + '/'
        self.save_file_path = self.output_dir + f'batch_{self.batch_size}_epoch_{self.max_epoch}_seed_{cfg.solver.seed}_fold_{cfg.set.ab_model}_alpha_{self.alpha}'
        if self.cfg.protein.use_bn:
            self.save_file_path = self.save_file_path + '_bn'
    ...
    def train(self):
        ...
        for i in range(self.init_epoch + 1, self.epochs + 1):
            ...
            # test_res = self.print_res('unseen_test_val')
